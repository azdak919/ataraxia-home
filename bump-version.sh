#!/usr/bin/env bash
# bump-version.sh — update the VERSION constant in js/version.js
...
if [[ ! -f "$VERSION_FILE" ]]; then
  echo "ERROR: js/version.js not found at $VERSION_FILE" >&2
  exit 1
fi
...
if [[ $# -ge 1 ]]; then
  ...
  CURRENT=$(grep -oP "(?<=const VERSION = ')[^']+" "$VERSION_FILE" || echo "unknown")
...
# Perform replacement
sed -i "s/const VERSION = 'v[^']*'/const VERSION = '$NEW_VERSION'/" "$VERSION_FILE"

# Verify
RESULT=$(grep "const VERSION" "$VERSION_FILE")
echo "Updated: $RESULT"

# Optionally tag and push
read -rp "Create git tag '$NEW_VERSION' and push? [y/N] " CONFIRM
if [[ "${CONFIRM,,}" == "y" ]]; then
  git -C "$SCRIPT_DIR" add js/version.js
  ...
  echo "Version updated in js/version.js. Stage and commit manually."
fi
